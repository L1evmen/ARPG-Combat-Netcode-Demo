...
本仓库的展示重点是 `Assets/Scripts` 与 `Assets/Editor` 中的游戏客户端业务代码、系统设计和验证流程。角色、场景、动画、音频、字体、贴图、模型以及部分 Shader/插件来自第三方资源或演示素材，不作为作者原创成果声明。
...
如果无法确认某项第三方资源允许公开再分发，应将源代码仓库保持私有，或在公开版本中用自制/明确授权的占位资源替换后再发布。B 站演示链接可用于展示完整视觉效果，不应以上传大型视频文件替代。

仓库未提供覆盖全部内容的统一开源许可证。保留第三方文件不代表授予任何额外使用权。
